...
var TemplateJS = function(){
	...
	View.prototype._installFocusHandler = function(fn) {
		...
		walkDOM(this.root,function(x){
			if (typeof x.tabIndex == "number" && x.tabIndex != -1 && !x.hidden && !x.disabled) {
	...
	function getElementOfItem(x) {
		if (x instanceof GroupManager) {
		    return x.baseEl;
		} else {
			return x;
		}
	...
	function updateSelectElement(elem, val) {
		if (typeof val == "object") {
			...
			if (Array.isArray(val)) {
				...
				for (var itm in val) {
					var opt = document.createElement("option");
					if (typeof val[itm] == "string") {
						opt.textContent = val[itm]  
					} else {
						opt.textContent = val[itm].label;
						if (val[itm].selected) {
			...
			if (!selchange) elem.value = curVal;
		} else {
			elem.value = val;
		}
	};
