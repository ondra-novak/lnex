var RpcClient = (function(){
	...
	var debug = !!localStorage["rpc_debug"]; 
	...
	RpcClient.enableDebug = function(e) {
		if (e) localStorage["rpc_debug"] = 1;
		else delete localStorage["rpc_debug"];
	};
					...
					if (jres.error) {
						return this.onError(jres.error).then(function(z) {
							if (z === undefined) return this.call2(method,args);
								...
								if (debug) console.log("RPC",method,args,z);
								return z;
							}
						}.bind(this), function(e){
							if (debug) console.error("RPC",method,args,e);
							throw e;
						});
					} else {
						if (jres.context) {
						...
						if (debug) console.log("RPC",method,args,jres.result);
						return jres.result;
					}
